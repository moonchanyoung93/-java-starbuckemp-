...
public class ListMember {
	public static void main(String[] args) {
		...
		list.add(new Member("choi", "1132", "최철수", "010-1234-5925", "choi@daum.net"));
		System.out.println(list.size());//list에 3개의 객체를 썻기에 3개가 나온다.
		System.out.println("이름\t아이디\t비번\t전화\t\t 이메일");
		for(int i =0; i<list.size(); i++) {
			Member m = list.get(i);//여기서 m은 ArrayList의 각 데이터를 가리틸 임시 참조변수로 활용
